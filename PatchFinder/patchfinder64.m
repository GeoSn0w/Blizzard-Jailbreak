...
InitPatchfinder(addr_t base, const char *filename)
{
    ...
    if (fd < 0) {
        printf(" [!] PatchFinder initialization failed at filename!\n");
        return -1;
    }
    ...
    if (rv != sizeof(buf)) {
        close(fd);
        printf(" [!] PatchFinder initialization failed at rv!\n");
        return -1;
    }
    ...
    if (!Kernel) {
        close(fd);
        printf(" [!] PatchFinder initialization failed at first step!\n");
        return -1;
    }
    ...
    for (i = 0; i < hdr->ncmds; i++) {
        ...
        if (cmd->cmd == LC_SEGMENT_64) {
            ...
            if (sz != seg->filesize) {
                close(fd);
                free(Kernel);
                printf(" [!] PatchFinder initialization failed at second step!\n");
                return -1;
            }
            if (!Kernel_mh) {
    ...
    (void)base;
    printf(" [+] PatchFinder initialization succeeded!\n");
    return 0;
}
